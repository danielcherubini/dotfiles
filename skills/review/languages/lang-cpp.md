...
> C++ code review guide focused on memory safety, lifetime, API design, and performance. Examples assume C++17/20/23.
...
```cpp
// ✅ C++23: Type-safe formatted output without format string issues
#include <iostream>
std::print("Hello, {}!\n", name);
std::println("Count: {:>5}", count);  // println adds newline
```

### std::generator (C++23, via P2449)
```cpp
// ✅ C++23: Coroutines for lazy sequences
#include <coroutine>
#include <ranges>
...
### std::expected (C++23)
```cpp
// ✅ C++23: Better error handling than exceptions for expected failures
#include <expected>
std::expected<int, ErrorCode> parse_int(std::string_view s) {
    try {
...
### ranges improvements
```cpp
// ✅ C++23: More convenient range algorithms
#include <ranges>
auto view = data | std::views::filter([](auto x) { return x > 0; })
